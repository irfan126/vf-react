...
export class EditableText extends EditableComponent {
	...
	renderComponentView() {
		...
		if (isActive) {
			return(
				<React.Fragment>
				<textarea onChange={(event) => this.handleChange(event)} value={value} className={className} rows={rows} cols={cols}></textarea>
		...
		return (
				...
				<button onClick={() => this.enableEdit() } className='btn btn-warning btn-editable' type='button'> Edit </button>
				</React.Fragment>		
			)
	}
